...
const STORAGE_SCORE_KEY = "scores";
...
sessionStorage.getItem(STORAGE_STATE_KEY) ? loadState() : resetGame();
...
function loadState() {
    state = JSON.parse(sessionStorage.getItem(STORAGE_STATE_KEY));
}
...
function saveScore() {
    const scores = JSON.parse(localStorage.getItem(STORAGE_SCORE_KEY)) || [];
    scores.push(state.score);
    scores.sort((a, b) => b - a);
    scores.splice(3);
    localStorage.setItem(STORAGE_SCORE_KEY, JSON.stringify(scores));
